...
library DXswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'd0d9bad4e674acbddd29729f10773af2aee1e9dee499c51bdf5ea12210740304' // init code hash
            ))));
    }
...
// File: contracts/interfaces/IDXswapRouter01.sol

pragma solidity >=0.6.2;

interface IDXswapRouter01 {
...
// File: contracts/DXswapRouter01.sol
...
contract DXswapRouter01 is IDXswapRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IDXswapPair(DXswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        uint amountIn,
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, DXswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint swapFee) public pure override returns (uint amountIn) {
        return DXswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut, swapFee);
    }
