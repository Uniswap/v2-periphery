...
describe.only('ExampleSwapToPrice', () => {
  ...
  describe('#swapToPrice', () => {
    ...
    it('moves the price to 1:90', async () => {
      await expect(swapToPriceExample.swapToPrice(token0.address, token1.address, 1, 90, wallet.address, MaxUint256))
    ...
    it('moves the price to 1:110', async () => {
      await expect(swapToPriceExample.swapToPrice(token1.address, token0.address, 110, 1, wallet.address, MaxUint256))
        .to.emit(swapToPriceExample, 'SwapToPrice')
